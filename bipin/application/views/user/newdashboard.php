		   ...
		   <?php }?>
            <div class="content">
               <div class="container">
                  ...
                  <div class="row">
                     <div class="col-sm-12">
                        <h4 class="page-title">vb starter dashboard san</h4>
                        <p class="text-muted page-title-alt"></p>
								<?php if(isset($paused_account[0])){
								...
								<h5 class="margintopzero" style="font-weight:700;"><?php echo $this->session->first_name; ?> has completed <span><?php //echo $mycompleted['complete_member']; ?></span> out of <span><?php //echo $mycompleted['total_member']; ?></span> assessments for this week</h5>
								<div class="col-sm-12 col-md-12 col-lg-12 col-xs-12" style="/*margin-bottom: 50px;*/">
									
									<?php //if($sender_receiver_same != 0){
										//if(abs($sender_receiver_same - $qarating) > 5){
											?>
											<!--</br>
											<button class="btn btn-danger checkpercentage" data-id="1"><?php echo $qarating; ?>%</button>
											<a class="checkpercentage_toggle1" style="display:none;" href="<?php echo base_url(); ?>addrating?id=<?php echo $this->session->user_id; ?>&sender_receiver=same&type=voice&sender_id=<?php echo $this->session->user_id; ?>&receiver_id=<?php //echo $qa_id; ?><?php //echo $query_string; ?>">
								...
								<div class="col-sm-8 col-md-8 col-lg-8 col-xs-12" style="/*margin-bottom: 50px;*/">
									...
									<div class="text-center textvoicebox col-sm-1 col-md-1 col-lg-1 col-xs-12">
										...
										<?php if(in_array($groupmember[$i]['id'],$given_rating_array_text['receiver_id']) && in_array($groupmember[$i]['id'],$given_rating_array_voice['receiver_id'])){ ?>
											...
											<?php } ?>
										<?php if(in_array($groupmember[$i]['id'],$given_rating_array_text['receiver_id'])){ ?>
										...
										<?php } ?>
										<?php if(in_array($groupmember[$i]['id'],$given_rating_array_voice['receiver_id'])){ ?>
									...
									<div class="col-sm-12 col-md-12 col-lg-12 col-xs-12 ">
										...
										<div class="textvoicebox col-sm-1 col-md-1 col-lg-1 col-xs-12">
											<h5 style="font-weight:700;"><?php echo $groupmember[$i]['first_name']; ?></h5>
											<h3><?php //echo $groupmember[$i]['assessed_percentage']; if($groupmember[$i]['assessed_percentage']!="NA") { echo "%"; } ?></h3>
											<?php if(in_array($groupmember[$i]['id'],$taken_rating_array_text['sender_id']) && in_array($groupmember[$i]['id'],$taken_rating_array_voice['sender_id'])){ ?>
												<i class="fa fa-user" style="width:100%;font-size: 90px;color: darkseagreen;"></i>
											<?php }else{ ?>
												<i class="fa fa-user" style="width:100%;font-size:90px;color: 2f2f2f;"></i>
                                                                                        <?php }//}else{ ?>
                                                                                               <!-- <i class="fa fa-user" style="width:100%;font-size:90px;color: 2f2f2f;"></i>-->
                                                                                        <?php //} ?>
											<?php if(in_array($groupmember[$i]['id'],$taken_rating_array_text['sender_id'])){ ?>
											...
											<?php }else{ ?>
												<button class="textpending btn" style="<?php echo $style; ?>">Text</button>
                                                                                        <?php }//}else{ ?>
                                                                                                <!--<button class="textpending btn" style="<?php echo $style; ?>">Text</button>-->
                                                                                        <?php //} ?>
											<?php if(in_array($groupmember[$i]['id'],$taken_rating_array_voice['sender_id'])){ ?>
												<a href="<?php echo base_url(); ?>addrating?id=<?php echo $groupmember[$i]['id']; ?>&type=voice&oww=yes<?php echo $query_string; ?>"	><button class="voicecomplete btn" style="background: #00a900;">Voice</button></a>
											<?php }else{ ?>
								...
								<div class="col-sm-4 col-md-4 col-lg-4 col-xs-12" style="float:right;">
											...
											<div class="col-sm-12 col-md-12 col-lg-12 col-xs-12 ownbg" style="padding:21px 0px 21px 21px;height:130px;">
												...
												<div class="col-sm-6 col-md-6 col-lg-6 col-xs-6">
													<span>
														<h1 style="color:#fff;font-weight:900;margin-bottom:0px;font-size: 45px;text-align:center;"><?php echo $squad_monthly_goal; ?>%</h1>
									...
									<div class="col-sm-6 col-md-6 col-lg-6 col-xs-12" style="padding-right: 0px;">
										<h4 style="font-size: 14px;color:#000;font-weight: 700;">MY WEEKLY GOAL:</h4>
					 ...
					 --->
								...
								<?php $col=3;
								if(sizeof($allweekdetails)==4){ $col=3; }
								if(sizeof($allweekdetails)==3){ $col=4; }
								if(sizeof($allweekdetails)==5){ $col=2; }
								for($i=0;$i<sizeof($allweekdetails);$i++){ ?>
								<?php if($i == sizeof($allweekdetails)-1){$padding_style='padding-right:0px;';}else{$padding_style = '';} ?>
								...
								<div class="col-sm-12 col-md-12 col-lg-12 col-xs-12 text-center" style="margin-top:15px;    margin-bottom: 200px;">
									<div class="col-sm-6 col-md-6 col-lg-6 col-xs-12 text-center">
										...
										<h1 style="color: #464545;font-size: 60px;font-weight: 900;">MY SQUAD</h1>
									...
									<div class="col-sm-6 col-md-6 col-lg-6 col-xs-12 text-center" style="padding-right:0px;">
										<div class="col-sm-12 col-md-12 col-lg-12 col-xs-12 text-center" style="background:#aef5ae;">
										<h1 style="color: #464545;font-size: 50px;font-weight: 900;">TOP SQUAD</h1>
										<h3 style="color: #464545;font-size: 35px;font-weight: 900;">FOR this week</h3>
										<div class="col-sm-6 col-md-6 col-lg-6 col-xs-6">
											<i class="fa fa-users" style="float:right;font-size:90px;"></i>
										</div>
...
<!-- All Model -->
<?php if(isset($_GET['week']) && $_GET['week']!="" && isset($_GET['month']) && $_GET['month']!="" && isset($_GET['year']) && $_GET['year']!=""){}else{ ?>
