...
class Squad extends CI_Controller {
    ...
    public function __construct() {
        ...
        if (!$this->ion_auth->in_group($group)) {
            ...
            $this->session->set_flashdata('message', 'You must be an Squad Member to view the user groups page.');
            redirect('auth/login');
        }
    ...
    public function index() {
        ...
        for ($i = 0; $i < 4; $i++) {
            ...
            $data_week = $this->backend->getWeek(array('from_date' => date("Y-m-d", $monday), 'to_date' => date("Y-m-d", $sunday)));
            if ($data_week) {
                $user_goal = $this->backend->get_table_row('weekgoal', array('user_id' => $this->logged_in_id, 'week' => $data_week->week, 'month' => $data_week->month, 'year' => $data_week->year));
                $week_goal[$i]['y'] = ($user_goal) ? $user_goal->weekgoal : 0;

                $peer_score = $this->calculate_love($user->id, $this->data['members'], $this->data['type'], $data_week);
                $qa_score = $this->calculate_qa_love($user, $this->data['type'], $data_week);
                $self_score = $this->calculate_self_love($user->id, $this->data['type'], $data_week);

                $peer_love = (($peer_score * 90) / 100) / 2;
                $qa_love = $qa_score / 2;
                $self_love = (($self_score * 10) / 100) / 2;

                $total = $peer_love + $qa_love + $self_love;
                $customer_love[$i]['y'] = $total;
                $customer_love[$i]['y'] = $total;
                $customer_love[$i]['label'] = 'Week ' . $data_week->week . '(' . date("F", mktime(0, 0, 0, $data_week->month, 1, 2011)) . ')';

                //var_dump($cus_love);
                //var_dump($qa_love);
                $perception_gap[$i]['y'] = $qa_score - $total;
                $perception_gap[$i]['label'] = 'Week ' . $data_week->week . '(' . date("F", mktime(0, 0, 0, $data_week->month, 1, 2011)) . ')';
            }
            $relative_time = $monday;
        }

        $this->data['datapoints'] = $perception_gap;
        $this->data['datapoints2'] = $week_goal;
        $this->data['datapoints3'] = $customer_love;

        // squad list
        $this->data['tribes'] = $this->backend->get_table('squad_group');

        $this->data['qa_list'] = $this->backend->get_table('qa_squad', array('squad_group' => $user->squad_group, 'week' => $week->week, 'month' => $week->month, 'year' => $week->year));

        // scheduled and feeds
        $this->data['scheduled'] = $this->backend->get_table('scheduled', array('user_id' => $this->logged_in_id, 'week' => $week->week, 'month' => $week->month, 'year' => $week->year));
