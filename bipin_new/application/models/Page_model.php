...
class Page_model extends CI_Model {
    ...
    public function login() {
        ...
        if ($data != 0) {
            ...
            if ($data != 0) {
                ...
                $this->session->set_userdata('user_name', $result[0]->first_name . ' ' . $result[0]->last_name);
                ...
                $this->session->set_flashdata('incorrect_info', 'Invalid username or password!', 5);
                redirect("login");
            }
        } else {
            $this->session->set_flashdata('incorrect_info', 'Invalid username or password!', 5);
    ...
    public function addrating($result) {
        if ($this->db->insert("rating", $result)) {
            return true;
        } else {
            return false;
        }
    ...
    public function addratingbycategory($result) {
        if ($this->db->insert("ratingbycategory", $result)) {
            return true;
        } else {
            return false;
        }
    ...
    public function addratingbyweek($result) {
        if ($this->db->insert("ratingbyweek", $result)) {
            return true;
        } else {
            return false;
        }
