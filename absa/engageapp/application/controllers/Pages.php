...
class Pages extends CI_Controller {
	...
	public function dashboard()
	{
		...
		/* New Portion Start For Graph */
		$allweekdetails_graph = $this->Admin_model->getstarttoendweekdetails_graph();
		$qarating_graph = array();
		//print_R($_SESSION); 
		for($i=0;$i<sizeof($allweekdetails_graph);$i++){
			$week = $allweekdetails_graph[$i]['week'];
			$month = $allweekdetails_graph[$i]['month'];
			$year = $allweekdetails_graph[$i]['year'];
			...
			$getqamember = $this->Admin_model->getqamember_graph($this->session->user_id,$week,$month,$year);
				if(!empty($getqamember)){
					if(isset($getqamember['assessed_percentage'])){
						$qarating_graph[$year][$month][$week] = $getqamember['assessed_percentage'];
						...
						if(isset($getqamember['text_voice_complete']) && $getqamember['text_voice_complete']==1){
							...
							if($count_user == 2){
								$sender_receiver_same_graph[$year][$month][$week] = ceil($mainpercentage_user/2);
								$qa_id = $getqamember['qa_id'];
							}else{
								$sender_receiver_same_graph[$year][$month][$week] = 0;
							}
						}
					}else{
						$qarating_graph[$year][$month][$week] = 0;
						$sender_receiver_same_graph[$year][$month][$week] = 0;
					}
				}else{
					$qarating_graph[$year][$month][$week] = 0;
		...
		$this->load->view('user/dashboard',array('data'=>$mycoin,
												'salt'=>$salt,
												...
												"qa_id"=>$qa_id,
												"sender_receiver_same_graph"=>$sender_receiver_same_graph,
												"qarating_graph"=>$qarating_graph));
	...
	public function forgot()
	{
		if(!empty($_POST)){
			...
			$headers = "From: noreply@geniusworks.co.za" . "\r\n";
